...
export default class PopUpModal extends React.Component {
  ...
  getAuthorData = async () => {
    ...
    if(item.attributes.author.data != null){
      const author = await axios.get(`/api/writers/${item.attributes.author.data.id}?populate=*`);
      this.setState({
      ...
      >
        <TouchableOpacity 
          ...
          <View style={[generalStyles.center]}>
            ...
            />
            <Text style={styles.title} onPress={() => closeModal()}>{this.state.authorInfo.name ?? 'The Author is Unknown'}</Text>
